...
let tempArray=[];
  useMemo(() => {
    const fetchPosts = async () => {
      let res = [];
      location.state !== null ? res = await axios.post('https://mernbackend-hatq.onrender.com/posts/title', { title: location.state }) : res = await axios.post(`/posts/pa`, { offset: offset });
      res.data.length === 0 ? setMessage(true) : setMessage(false);
